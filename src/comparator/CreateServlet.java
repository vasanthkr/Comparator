...
//		if(command.equals("z")){ //shabab 
//			User u = dao.ofy().find(User.class, 113);
//			//{11128, 10105, 13120, 11127, 9118}
//			List<Long> bin = new ArrayList<Long>();
//			bin.add(Long.valueOf(11128));
//			bin.add(Long.valueOf(10105));
//			bin.add(Long.valueOf(13120));
//			bin.add(Long.valueOf(11127));
//			bin.add(Long.valueOf(9118));
//			
//			u.setSortedList(bin);
//			dao.ofy().put(u);
//			Rating r = dao.ofy().find(Rating.class, 10105);
//			r.setType("listL");
//			dao.ofy().put(r);
//			r = dao.ofy().find(Rating.class, 9118);
//			r.setType("listL");
//			dao.ofy().put(r);
//		}
//		else 
			if(command.equals("q")){ //create users
			try {
				for(int i = 101; i<117; i++){
					User u = dao.ofy().find(User.class, i);
					User u2 = new User();
					u2.setBinaryList(u.getBinaryList());
					u2.setId(u.getId()+100);
					u2.setMovies(u.getMovies());
					u2.setName(u.getName());
					u2.setRatings(u.getRatings());
					u2.setRatingsX(u.getRatingsX());
					u2.setSequence(u.getSequence());
					u2.setSortedList(u.getSortedList());
					dao.ofy().put(u2);
				}
